...
# Определяем базовые пути для production
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
INSTANCE_DIR = os.path.join(BASE_DIR, 'instance')

# Создаем instance директорию, если её нет
...
# Настройка пути к базе данных для production
# Переопределяем путь к БД, если нужно (можно через переменную окружения)
if 'DATABASE_PATH' in os.environ:
    app.config['DATABASE_PATH'] = os.environ['DATABASE_PATH']
else:
    # По умолчанию БД в instance директории
    app.config['DATABASE_PATH'] = os.path.join(INSTANCE_DIR, 'todo.db')

# Инициализируем приложение для работы с БД (закрытие соединений)
init_db_app(app)

# Функция для получения пути к БД (для использования в других модулях)
def get_db_path():
...
# Инициализация базы данных при запуске в production
...
if __name__ == "__main__":
    ...
    initialize_database()
    print(f"Запуск сервера разработки на http://0.0.0.0:5000")
    app.run(debug=True, host='0.0.0.0', port=5000)
else:
    # Для production (Gunicorn)
    # Инициализируем БД при старте приложения в контексте
    with app.app_context():
        initialize_database()
    print("Приложение загружено для production режима")